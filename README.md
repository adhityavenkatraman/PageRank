...
All of the results below have been updated to reflect the following changes. I updated the pagerank.py implementation to expand the score of the --search_query argument. Rather than finding urls that have only that term in them, the updated program uses the gensim glove-twitter-25 model to identify the top 5 most similar words. These additional words are then included as queries as well, helping to find related articles that may happen to not include the query we directly identified. For example, the ranking associated with a query for "pets" might include links that reference "dogs". The results below have been updated to this new ranking system. 
