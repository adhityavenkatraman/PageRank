    ...
    def power_method(self, v=None, x0=None, alpha=0.85, max_iterations=1000, epsilon=1e-6):
        ...
        with torch.no_grad():
            ...
            for k in range(0,max_iterations):
                ...
                vectorterm1 = (self.P.t()@alphax.t()).t()    #((nxn)T mtimes (1xn)T)T    #mtimes=matrixmultiplication
                ...
                x = (vectorterm1 + vectorterm2).t()    #add terms and take the transpose of the final pi vector
                accuracy=torch.norm(x-xold).item()
                if accuracy < epsilon:    #accuracy condition
                    break
            return x
